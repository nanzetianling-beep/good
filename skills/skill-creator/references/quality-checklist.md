...
- [ ] Max 64 characters; contains no XML tags.
- [ ] Does not contain reserved words `anthropic` or `claude`.
- [ ] **Matches the parent directory name exactly.** In Claude Code the directory name is authoritative (it becomes the `/command`; `name` defaults to it), and the Agent Skills standard / API packaging require `name` to match the folder — keep them identical.
...
- [ ] States **when** to use it, with concrete trigger words and file types. All "when to use" info lives here, not in the body.
- [ ] "Pushy" enough that Claude reliably selects it for the intended tasks (counters under-triggering).
...
- [ ] Concise — no explanation of things Claude already knows; each paragraph justifies its token cost.
- [ ] Degree of freedom matches task fragility (prose for flexible, exact commands for fragile).
- [ ] Explains the *why* behind instructions rather than piling on all-caps MUSTs.
...
- [ ] All references are **one level deep** from SKILL.md (no reference-to-reference chains that trigger partial reads).
- [ ] Reference files over ~100 lines start with a table of contents.
- [ ] All paths use forward slashes.
- [ ] Files are named by content (`form-validation-rules.md`, not `doc2.md`) and organized by domain.
...
- [ ] Required packages listed; no assumption that tools are pre-installed (Claude API has no network access).
- [ ] MCP tools referenced by fully qualified name (`ServerName:tool_name`).
- [ ] For high-stakes batch operations, use a plan-validate-execute pattern with a verifiable intermediate file.

## Evaluation and iteration

- [ ] A baseline was established first — Claude run on representative tasks without the skill to find real gaps (eval-driven development).
- [ ] At least 3 realistic test prompts written (what a real user would type: file paths, casual phrasing, typos — not abstract).
- [ ] Skill run against each prompt and compared to the baseline (or the previous version, when improving an existing skill).
...
- [ ] Revisions generalize from feedback rather than overfitting to one test case; the prompt stays lean.
- [ ] `description` optimized for triggering in a final pass: should-trigger and should-not-trigger (near-miss) prompts confirm it fires only when intended.
...
- Documenting imagined problems instead of gaps observed in a baseline run.
