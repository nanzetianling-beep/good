---
name: plugin-dev
description: Builds and packages Claude Code plugins that bundle commands, skills, agents, hooks, and MCP servers behind a .claude-plugin/plugin.json manifest, then distributes them via a marketplace.json or a ZIP archive. Use when a user wants to build a plugin, scaffold plugin.json, wire an MCP server or API integration into a plugin, create a plugin marketplace, or bundle in-house tooling into a shareable package.
---
...
A Claude Code **plugin** is a self-contained directory that extends Claude Code with reusable functionality: skills, agents, hooks, MCP servers, LSP servers, and background monitors. Everything is described by a single manifest, `.claude-plugin/plugin.json`. Because a plugin can bundle **MCP servers** (external tool/API connections) alongside the **skills and commands** that drive them, it is the right unit for packaging an in-house tool that integrates with an internal system and sharing it across a team — as a git-hosted marketplace or as a `.zip` archive.
...
Only `plugin.json` lives inside `.claude-plugin/`. **Every other directory sits at the plugin root** — a common mistake is nesting `skills/` or `.mcp.json` inside `.claude-plugin/`, which breaks the plugin.
...
Components in these default locations are auto-discovered — the manifest only needs to point at components in **non-default** paths. See `references/manifest-schema.md` for every manifest field.

### Manifest fields (`.claude-plugin/plugin.json`)

`name` is the only required field (kebab-case, no spaces; it becomes the namespace prefix). Common fields:
...
| `mcpServers` | Inline MCP config or path(s) (default `.mcp.json` is auto-loaded). |
| `commands`, `agents`, `skills`, `hooks`, `lspServers` | Only needed for custom (non-default) paths. |
| `userConfig` | Values Claude Code prompts for at enable time (API endpoints, tokens). |
| `defaultEnabled` | `false` ships the plugin installed-but-disabled (good for plugins that hit external services). |
...
Always reference bundled files with `${CLAUDE_PLUGIN_ROOT}` (the absolute install path — it changes on update, so never hardcode paths). Prompt for secrets via `userConfig` and inject them as `${user_config.KEY}` rather than committing tokens.
...
MCP servers a plugin declares go through the same per-server approval as a project `.mcp.json`. See `templates/.mcp.json` and `templates/plugin.json`.

## Step-by-step

### 1. Scaffold
Fastest: `claude plugin init my-tool` scaffolds `~/.claude/skills/my-tool/` with a manifest + starter SKILL.md that auto-loads next session (no marketplace). For a distributable plugin, create the tree by hand:
```bash
...
- **Agents**: `.md` files in `agents/`. Note: plugin agents may **not** declare `hooks`, `mcpServers`, or `permissionMode`.
- **Hooks**: `hooks/hooks.json` (same schema as `.claude/settings.json` hooks). Reference scripts as `"${CLAUDE_PLUGIN_ROOT}"/scripts/x.sh`.

### 4. Add MCP servers
Create `.mcp.json` at the plugin root (see template). Use `${CLAUDE_PLUGIN_ROOT}` for bundled server paths and `${user_config.*}` for endpoints/secrets declared in `userConfig`.

### 5. Test / install locally
```bash
claude --plugin-dir ./my-plugin        # load without installing (also accepts a .zip)
claude plugin validate ./my-plugin     # validate; add --strict for CI
```
After edits run `/reload-plugins` (SKILL.md changes apply live; hooks/MCP/agents need a reload or restart). Verify: `/my-plugin:skill-name`, agents under `/context`, hooks fire, MCP tools appear.
...
```
Host in a **private repo** to keep it internal. See `templates/marketplace.json`.

**ZIP (quick sharing, no repo).** Zip the plugin directory and share the file. Recipients load it with `claude --plugin-dir ./my-plugin.zip` (needs Claude Code v2.1.128+), or serve it and use `claude --plugin-url https://.../my-plugin.zip`. Use `scripts/package.sh` to build the archive.
...
- Never hardcode absolute paths — always `${CLAUDE_PLUGIN_ROOT}`. Don't write state there (it's replaced on update).
- Never commit secrets; use `userConfig` with `"sensitive": true` (stored in the keychain, injected as `${user_config.KEY}`).
- A plugin's cache copy can't reference files outside its own directory (`../shared`); use symlinks if you must share.
...
- Plugins reference (manifest schema, MCP servers, versioning) — https://code.claude.com/docs/en/plugins-reference
